...
public:
  ...
  : Node("collision_object_manager"),
    ...
    attach_link_(declare_parameter<std::string>("attach_link", "bob_robotiq_85_right_finger_tip_link"))
  {
    // Allow configuring touch links via params
    touch_links_ = declare_parameter<std::vector<std::string>>(
        "touch_links",
        std::vector<std::string>{
          "bob_robotiq_85_left_finger_tip_link",
          "bob_robotiq_85_right_finger_tip_link"
        });
    ...
    // bob_Gripper status → attach/detach
    bob_gripper_status_sub_ = this->create_subscription<std_msgs::msg::String>(
        "/bob_gripper/status", 10,
        std::bind(&CollisionObjectManager::bob_gripperStatusCallback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(this->get_logger(),
                "CollisionObjectManager ready (default_object_id='%s', attach_link='%s')",
                default_object_id_.c_str(), attach_link_.c_str());
  }

private:
  void bob_gripperStatusCallback(const std_msgs::msg::String::SharedPtr msg)
  {
    const std::string& status = msg->data;

    if (status.find("[BOB gripper] bob_gripper closed successfully") != std::string::npos)
    {
      attachObject();
    }
    else if (status.find("[BOB gripper] bob_gripper fully opened successfully - object released") != std::string::npos)
    {
      detachObject();
    }
  }
  ...
  // ROS / MoveIt
  rclcpp::Subscription<std_msgs::msg::String>::SharedPtr bob_gripper_status_sub_;
